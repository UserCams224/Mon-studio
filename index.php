...
$reservation_message = "";
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['reserver']) && $conn) {
    ...
    if (empty($errors)) {
        try {
            ...
            $reservation_message = '<div class="error-message">Erreur de base de données. Veuillez réessayer plus tard.</div>';
...
if (isset($_SESSION['admin_logged_in']) && $_SESSION['admin_logged_in'] === true) {
    ...
    // Récupérer les réservations si la connexion à la base est établie
    if ($conn) {
        try {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['login'])) {
    ...
    // Vérification des identifiants (codés en dur pour Github)
    // Note: En production, utilisez des identifiants sécurisés et stockez-les dans la base de données
...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Section Hero */
        .hero {
            background: linear-gradient(rgba(10, 10, 20, 0.85), rgba(10, 10, 20, 0.9)), url('https://images.unsplash.com/photo-1511379938547-c1f69419868d?ixlib=rb-4.0.3&auto=format&fit=crop&w=2070&q=80');
...
<body>
    <!-- Notice pour le déploiement Github -->
    <?php if (!$conn): ?>
    <div class="demo-notice">
        ⚠️ Mode démo - La connexion à la base de données n'est pas configurée. 
        Pour une installation complète, configurez les paramètres de connexion à la base de données.
    </div>
    <?php endif; ?>
